...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    public Set<E> subSet(E element1, E element2) {
        if (element1 == null || element2 == null) {
            throw new IllegalArgumentException("element1 is null or element2 is null in subSet(E element1, E element2)");
        }

        BstNode<E> newRoot = null;

        BstNode<E> node = root;
        while (node != null) {
            int cmp = c.compare(element1, node.element);
            if (cmp < 0) {
                node = node.left;
            } else if (cmp > 0) {
                node = node.right;
        ...
        Set<E> set = new BstSet<>();
        while (newRoot != null) {
            set.add(newRoot.element);
            int cmp = c.compare(element2, newRoot.element);
            if (cmp < 0) {
    ...
    public Set<E> tailSet(E element) {
        ...
        set.root = cloneRecursive(newRoot);
