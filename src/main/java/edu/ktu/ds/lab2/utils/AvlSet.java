...
public class AvlSet<E extends Comparable<E>> extends BstSet<E>
    ...
    private AVLNode<E> removeRecursive(E element, AVLNode<E> node) {
        ...
        if (cmp < 0)
            ...
            if ((node.left == null) || (node.right == null)) {
                ...
                if (temp == node.left)
                    temp = (AVLNode<E>) node.right;
                else
                    temp = (AVLNode<E>) node.left;
                ...
                AVLNode temp = minValueNode((AVLNode<E>) node.right);
                node.element = (E) temp.element;
                node.right = removeRecursive((E) temp.element, (AVLNode<E>) node.right);
            }
        }
        ...
        node.height = max(height((AVLNode<E>) node.left), height((AVLNode<E>) node.right)) + 1;

        int balance = getBalance(node);

        if (balance > 1 && getBalance((AVLNode<E>) node.left) >= 0)
            return rightRotation((AVLNode<E>) node);

        if (balance > 1 && getBalance((AVLNode<E>) node.left) < 0)
        {
            root.left = leftRotation((AVLNode<E>) node.left);
            return rightRotation(node);
        }

        if (balance < -1 && getBalance((AVLNode<E>) node.right) <= 0)
            return leftRotation(node);

        if (balance < -1 && getBalance((AVLNode<E>) node.right) > 0)
        {
            root.right = rightRotation((AVLNode<E>) node.right);
            return leftRotation(node);
        }
    ...
    private AVLNode minValueNode(AVLNode node)
    {
        ...
        while (current.left != null)
            current = (AVLNode<E>) current.left;
